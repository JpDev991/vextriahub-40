...
const Audiencias = () => {
  ...
  const handleDeleteSelected = () => {
    ...
    const realizedOrPast = selectedItems.filter(audiencia => 
      audiencia.status === "realizada" || new Date(audiencia.data) < new Date()
    );
    
    if (realizedOrPast.length > 0) {
      toast({
        title: "Não é possível excluir",
        description: "Audiências realizadas ou passadas não podem ser excluídas.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const updatedAudiencias = audiencias.filter(audiencia => !selectedIds.includes(audiencia.id));
      setAudiencias(updatedAudiencias);
      
      toast({
        title: "Audiências excluídas",
        description: `${selectedItems.length} audiência(s) foram excluída(s) com sucesso.`,
      });
      multiSelect.clearSelection();
    } catch (error) {
      toast({
        title: "Erro ao excluir",
        description: "Ocorreu um erro ao excluir as audiências.",
        variant: "destructive",
      });
    } finally {
      setIsDeleting(false);
