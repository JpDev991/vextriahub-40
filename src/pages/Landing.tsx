...
const Landing = () => {
  ...
  // Limpar dados antigos e cache na primeira visita
  useEffect(() => {
    // Limpar dados antigos do sistema anterior
    const oldKeys = ['nublex_token', 'nublex_user', 'nublex_data', 'old_auth_token'];
    oldKeys.forEach(key => localStorage.removeItem(key));
    
    // Force refresh para limpar cache do navegador se necessário
    const hasRefreshed = sessionStorage.getItem('hasRefreshed');
    if (!hasRefreshed) {
      sessionStorage.setItem('hasRefreshed', 'true');
      // Opcional: Force refresh apenas se detectar dados antigos
      if (localStorage.getItem('nublex_token') || document.title.includes('nublex')) {
      ...
      <section id="planos" className="py-20">
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            ...
            <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
              Comece com 14 dias grátis em qualquer plano. Mude de plano quando quiser, sem complicação.
            </p>
          </div>
                ...
                <CardContent className="space-y-4">
                  <ul className="space-y-2">
                    ...
                    <li className="flex items-center gap-2 text-sm">
                      ...
                      <span className={plan.ai ? '' : 'text-muted-foreground'}>IA (futura)</span>
                    </li>
                  </ul>
                  <Button 
                    className="w-full" 
      ...
      <footer className="border-t border-border bg-card py-12">
        <div className="container mx-auto px-4">
          ...
          <div className="border-t border-border mt-8 pt-8 text-center text-muted-foreground">
            <p>© 2024 VextriaHub. Todos os direitos reservados.</p>
          </div>
        </div>
      </footer>
    </div>
